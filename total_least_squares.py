...
def f3():
    ...
    plt.plot(x_fit_high,y_fit_high,'.', label='TLS_continuous_high')

    x_fit_low = np.linspace(0, np.max(x_regular), 1000)
    y_fit_low = a_tls_low[0] + a_tls_low[1] * x_fit_low + a_tls_low[2] * x_fit_low**2
    plt.plot(x_fit_low,y_fit_low,'.', label='TLS_continuous_low')

    plt.plot(x_regular,y_high_n,'.', label='data_high')
    plt.plot((X_high+Xt_high)[:,1], y_tls_high,'+', label='TLS_discrete_high')

    plt.plot(x_regular,y_low_n,'.', label='data_low')
    plt.plot((X_low+Xt_low)[:,1], y_tls_low,'x', label='TLS_discrete_low')
